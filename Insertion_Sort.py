...
class InsertionSortVisualizer:
    def __init__(self, root):
        ...
        self.current_state = {
            "type": None,
            "array": [],
            "comparing": None,  # (j, i) indices being compared (j: sorted portion, i: key)
            "inserting": None,  # (insert_idx, key) for insertion
            "pass_num": 0
        }
    ...
    def start_sorting(self):
        """Start the insertion sort visualization process."""
        if self.is_sorting:
    ...
    def insertion_sort_with_viz(self):
        """Perform insertion sort with visualization steps, inserting key in one step."""
        # Store initial state
        state = {
            "type": "initial",
            "array": self.array[:],
        ...
        for i in range(1, n):
            key = current_array[i]
            j = i - 1
            insert_pos = i  # Default insertion position

            # Comparison steps to find insertion position
            while j >= 0:
                state = {
                    "type": "compare",
                    "array": current_array[:],
                    "comparing": (j, i),  # Compare current element (j) with key (at i)
                    "inserting": None,
                    "pass_num": i
                }
                self.sorting_steps.append((self.draw_state, state))

                if current_array[j] <= key:
                    break
                insert_pos = j  # Update insertion position
                j -= 1

            # Perform insertion in one step
            if insert_pos != i:
                # Shift elements from insert_pos to i-1 one position to the right
                temp = current_array[insert_pos:i]
                current_array[insert_pos + 1:i + 1] = temp
                current_array[insert_pos] = key
                state = {
                    "type": "insert",
                    "array": current_array[:],
                    "comparing": None,
                    "inserting": (insert_pos, key),
                    "pass_num": i
                }
                self.sorting_steps.append((self.draw_state, state))
    ...
    def draw_state(self):
        ...
        if self.current_state["type"] == "compare" and self.current_state["comparing"]:
            j, i = self.current_state["comparing"]
            if 0 <= j < len(colors) and 0 <= i < len(colors):
                colors[j] = self.active_color
                colors[i] = self.active_color
        elif self.current_state["type"] == "insert" and self.current_state["inserting"]:
            insert_idx, _ = self.current_state["inserting"]
            if 0 <= insert_idx < len(colors):
                colors[insert_idx] = self.active_color
        ...
        if self.current_step < len(self.sorting_steps) - 1:
            if self.current_state["type"] == "compare" and self.current_state["comparing"]:
                ...
                self.tree_draw_text(
                    400, 90,
                    text=f"Comparing index {j} with key at index {i}",
                    font=("Helvetica", 20, "bold"),
                    fill="black",
                    anchor="center"
                )
            elif self.current_state["type"] == "insert" and self.current_state["inserting"]:
                insert_idx, key = self.current_state["inserting"]
                self.tree_draw_text(
                    400, 90,
                    text=f"Inserting {key} at index {insert_idx}",
                    font=("Helvetica", 20, "bold"),
                    fill="black",
                    anchor="center"
                )
...
if __name__ == "__main__":
    ...
    image_path = os.path.join(images_dir, "images", "insertion_sort.png")
